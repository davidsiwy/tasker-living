...
export function LanguageSwitcher({ variant = 'pills' }: { variant?: 'pills' | 'select' }) {
  ...
  if (variant === 'select') {
    return (
      <select aria-label="Language" value={current} onChange={(e) => pick(e.target.value as Lang)} className="lang-select">
